...
-   `dpkg --print-architecture`: retrieve architecture, for package installation[^installation-linux-1].
-   {{< kbd Ctrl-H >}} in **Gnome**, aka File Management, to list hidden files

[^installation-linux-1]: Returns `amd64` in my case
...
-   Recent modern routers return only IPv6 -\> while Windows is able to able to auto-fall back to IPv4, this is not the case of IPv6.
    ...
    -   *Permanent fix*[^installation-linux-2]:
...
[^installation-linux-2]: Alternatively, you can rely on the GNOME Interface, `Wifi` section
...
i.  Exclude large folders, and directories that are complex and are not recommended for syncing, `nano ~/.config/onedrive/config`. You can see the actual content of my config file under [onedrive-custom-config](../Config/onedrive-config).[^installation-linux-3]

ii. Authenticate, and run OneDrive once

[^installation-linux-3]: Note that **Comments must be placed in separated lines, not inline**.
...
i.  Configure globally the `.gitconfig` configuration file to allow both end of lines indicators, using `git config --global core.autocrlf input` on Linux and Mac machines collaborating with Windows machines[^installation-linux-4], and `git config --global core.autocrlf true` on Windows machines[^installation-linux-5].

ii. For further robustness, create a `.gitattributes` dotfile per project[^installation-linux-6]:
...
[^installation-linux-4]: Accept as EOL CRLF, but always commit as LF

[^installation-linux-5]: constrain commits to be encoded and pushed as `LF`

[^installation-linux-6]: Detailled information is avalaible [here](https://docs.github.com/en/get-started/git-basics/configuring-git-to-handle-line-endings?platform=windows)
...
1.  If supported by Ubuntu, go for `apt install` for system software[^installation-linux-7]:
...
-   If **not directly available on Ubuntu’s native package manager, or using in the back-end `snap` commands**, switch to pre-built tarball installation[^installation-linux-8], or add the repository (only if **trusted**) to `apt allowed keys` folder. Example for `Docker`, @sec-Docker, or [custom Zotero installation file](https://github.com/retorquere/zotero-deb/blob/master/install.sh).
...
[^installation-linux-7]: `apt-get` works similarly, but is now deprecated

[^installation-linux-8]: Fetch the latest stable archive, extract it, and add executables to your `PATH`, see example under @sec-Zotero
...
i.  For HTTPS + PAT token protocol, use `git config --global credential.helper store` for **permanent** registration of the password (should we not encrypt the file??), or `git config --global credential.helper 'cache --timeout=3600'` for temporary access[^installation-linux-9].
ii. Define SSH key.

[^installation-linux-9]: Time-out units are reported in seconds, accordingly, this amounts to one hour
...
1.  Relocate everything under **`/opt/sioyek`**[^installation-linux-10]. \*\*Note that the structure of a package may not follow exactly this of `sioyek`!!
...
[^installation-linux-10]: default repo for third-party apps, preventing conflicts with distributed, native packages
...
ii. Install dictionaries + extension [Grammalecte and LanguageTool](https://doc.ubuntu-fr.org/correction_grammaticale)[^installation-linux-11]. *needs to check if LanguageTool is truly relevant, is AI-based, but also resource-consuming*.

[^installation-linux-11]: You may configure the Java version used by default under `Tools -> Options -> Advanced`
...
ii. Extra fonts installation[^installation-linux-12]. Most popular fonts can be found under [Every-Font GH repository](https://github.com/serendipious/every-font/):

[^installation-linux-12]: Alternatively, you also have the native Fonts manager tool under GNOME launch start
...
-   [Benjamin presentation](https://docs.google.com/presentation/d/1v6JjkoY2ozFO0RaCBMIVRX5Hy7UW6CuhBgORrlM5pHQ/edit?slide=id.g2b840ea5086_0_12#slide=id.g2b840ea5086_0_12)
-   [VsCode, R and Docker with DevContainers](https://github.com/RamiKrispin/vscode-r)
-   [VsCode, Python and Docker with DevContainers](https://github.com/RamiKrispin/vscode-python)
-   [DevContainers, from INRIA trainings](https://gitlab.lis-lab.fr/sicomp/and_devcontainers)
...
1.  Install Java version 17 with `sudo apt install openjdk-17-jdk`[^installation-linux-13]
2.  Follow this [tutorial](https://docs.docker.com/engine/install/ubuntu/#install-using-the-repository)

[^installation-linux-13]: On Ubuntu, a more modern version of Java, the 21, was installed
...
2.  Retrieve the latest stable version on <https://www.zotero.org/download/>[^installation-linux-14].

3.  Run following instructions + Synchronise by installing Zotero Connector on your website:

[^installation-linux-14]: Alternatively, download with `curl`
...
ii. Edit Zotero preferences[^installation-linux-15]: - Uncheck the option to create automatic web page snapshots (increases cluttering with lots of small files added) - Uncheck full-text sync - Set apart in `Edit->Settings->Advanced` the
...
[^installation-linux-15]: What matters the most here is uniformity across platforms

```         
![Zotero Database Configuration](zotero-advanced-configuration.png)
```

iii. Edit `Better BixTex` *plug-in*: - Personally, use `[auth:lower][year][journal:lower:abbr]` - Should be the same across machines.
...
2.  **Identify what it's worth relevant for syncing**: indeed, while you could export all the RStudio settings for a complete backup, most of them are optional, or barely modified by regular users. Instead, I would identify, and retrieve only the settings explicitly changed by the user. \> Would use local, or online diff editors, such as [diffcheckers](https://www.diffchecker.com/) to quickly identify what changed between personal configurations. \> Specifically, all external tools that complement the functionalities of RStudio, such as pdf previewer, or external formatters such as Air, are hard-coded as absolute paths, and are accordingly highly specific to a personal laptop.

3.  **Homogenise settings across machines**: once you've finalised the customisation of the RStudio settings to your specific OS configuration, simply overwrite pre-existing *user-specific* settings file `rstudio-prefs.json`. **Restart RStudio** to check if changes were properly integrated into the graphical interface[^installation-linux-17].

> To simplify the syncing across machines (especially for teams with shared computing environment), use either: 1. Git-based sync, storing your dotfiles. This is the recommended approach. You can find my settings files under [`rstudio-prefs.json`](../Config/RStudio/rstudio-prefs.json). See also @fig-Rstudio-options for a more comprehensive explantion of core changes brought to a default, native RStudio configuration. 2. use symbolic links, or cloud sync (but you have fewer options to adjust for changes)
...
[^installation-linux-16]: `%AppData%` is a shortened alias for this expanded path on Windows: `C:\Users\<YourUser>\AppData\Roaming\RStudio`

[^installation-linux-17]: If some of the setting changes were not applied, you can usually access them from the Menu options with `Tools > Global Options...`
...
2.  (optional, but recommended) Use `radian`as enhanced R terminal with `uv tool install radian`

3.  Install the `vscDebugger` extension for VS Code, and run the [debug demo](../Linux/R/debug_demo.R) script to test it.

![Side by side comparison of the debug demo script, and the debugger in action](debugger_R.png){fig-align="center"}
...
![Main commands to replace old-fahsioned Python tools by `uv`.](uv%20cheatsheet.png)
...
uv tool install "dvc[ssh]" # recommended data management tool, and versining, if exceedging git-lfs
...
4.  Interactive example is provided in [demo.py](../Linux/marimo/demo.py).
...
3.  Sync your Cursor configuration across devices, see @tip-cursor-output:

::: {#tip-cursor-output .callout-tip title="Export Cursor settings"}
1.  Export the core settings:
    1.  Open Command Palette (`Ctrl/Cmd + Shift + P`), then search: **“Preferences: Open Settings (JSON)”**
    2.  Export the file in your new device
    3.  Default Linux installation: `~/.config/Cursor/User/settings.json`
2.  Export the keybinding shortcuts:
    1.  Open: **“Preferences: Open Keyboard Shortcuts (JSON)”**
    2.  Export `keybindings.json` into your local folder: `~/.config/Cursor/User/keybindings.json`
3.  (**the main difference with VSCode**). Contrary to `VSCode`, you need to export the list of extensions manually:
    i.  Retrieve the list of extensions with `cursor --list-extensions > extensions.txt`. To also record each extension’s version, use `cursor --list-extensions --show-versions > extensions.txt`
...
> More details are provided in [*Sync Cursor Settings the Dotfiles Way* website](https://dev.to/0916dhkim/sync-cursor-settings-the-dotfiles-way-20c9), including the symlink commands to fully automate this process

> The hardcore way, for syncing configuration files across numerous users, cf Benjamin, and [Cursor Config Sync](https://open-vsx.org/extension/KushalCodes/cursor-config-sync)
:::
...
> Follow the following steps for its installation[^installation-linux-18]

[^installation-linux-18]: Contrary to best guidelines suggested in @nte-sofware-installation, only the `flatpak` and `snap` installation are supported for Ubuntu devices -\> no automatic updates with native `apt` Ubuntu dependency manager
...
3.  **Log-in using your university email address, using `SSO`.** The account is provided with unlimited meetings available by default, use following domain: `univ-amu-fr`. [^installation-linux-19]

[^installation-linux-19]: Alternatively, use the web version available [here](https://univ-amu-fr.zoom.us/meeting#/upcoming)
...
> More information avalaible under [Server Management tutorial](../Server%20Management/Server_Management.pdf).
:::
...
2.  Generate a pair of public-private SSH key on your local machine[^installation-linux-20]

[^installation-linux-20]: Using a passphrase is not compuslory, but safer
...
<!--     /home/bastienc/project  <--SSH-->

\~/remote_project --\> <!--     ## 3-layered structure mounting --> <!--     Application (RStudio, editor) --> <!--             │ --> <!--             ▼ --> <!--     FUSE filesystem --> <!--             │ --> <!--             ▼ --> <!--     SSHFS driver --> <!--             │ --> <!--             ▼ --> <!--     SSH connection --> <!--             │ --> <!--             ▼ --> <!--     Remote filesystem (server) --> <!--    ``` -->
...
Finally, if you: - Want to collaborate to the tutorial: favour quarto (or notebooks) editable files over PDFs, index your document under `https://github.com/BAUDOTlab/teaching_material/blob/main/_quarto.yml`. You can also report issues directly on the website, clicking on the option "Edit this page" in the footer - For computational nerds, or people generally interested in best programming guidelines, solving computational and dependency nightmares, or simply curious, add a like to this comment -\> I'll add a Slack channel to unify debates, and troubleshoot issue solving :-)
