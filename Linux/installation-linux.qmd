...
7. (Optional) Visual interaction of OneDrive synchronization:
  i. [OneDrive system tray program](https://github.com/DanielBorgesOliveira/onedrive_tray) adds a simple icon returning the status of synchronization for OneDrive, that can be controlled with by clicking with the right button of the mouse. 
...
5. Add plug-in `Better BibTeX` for custom exports of plain .bib files, tailored for distinct conferences formats:

  - [Plug-in installation tutorial](https://retorque.re/zotero-better-bibtex/installation/index.html)
  - My custom `.bib` output is stored under [Better BibTeX preferences](../Config/better_bibtex_preferences.json). You can import it by going down to the `Settings/Better Bibtex` menu.
  - Should be the same output across machines. 
  
6. Recommended Zotero configurations^[*What matters the most here is uniformity across platforms]: 
    - Uncheck the option to create automatic web page snapshots (increases cluttering with lots of small files added)
    - Uncheck "Sync attachment files in My Library" in the Sync settings if you are on a free version of Zotero's native cloud file storage (limited to 500Mo only)


6. (**Optional**) Enable **unlimited synced storage** for articles on multiple machines,  
  -  [`Zotmoov` installation](https://github.com/wileyyugioh/zotmoov/blob/master/README.md#installation) extension now replaces `Zotfile` (no longer maintained) for renaming files, and is required for this renaming trick
  - Tick in `Edit->Settings->ZotMoov->Advanced Options` "Strip diacritics from File Names" option 
  - Other important options are listed in following figure. Notably: 
    - Set the directory to move in a synced repository such that it can be shared across devices.
    - Choose wildcard for ordering, and renaming files -> in my case, one folder per first author. 
  - **Once done, `Ctrl+A` to select all documents, then Right click, then `ZotMoov: Move to Selected Directory` to update the localisation of all documents (otherwise, not modified automatically)**
  - **Also note that this trick is not working for Shared groups**

![`ZotMoov` core options](Zotmov_preferences.png)

7. Install [`Inciteful`](https://github.com/inciteful-xyz/inciteful-zotero-plugin), then Right Click to access to the connected graph of papers
