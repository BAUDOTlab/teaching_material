...
For the full appendix (commands, GitHub UI, advanced topics, and
training exercises), see
[Git and GitHub](../Git/git-guidelines.html).

![Git general workflow](git/git-workflow.png){#fig-git-workflow}
...
3.  General options to configure:
    ...
    ii. Or edit `~/.gitconfig` directly (more prone to indentation
        ...
        for a fuller description of options, see the
        [Git and GitHub appendix](../Git/git-guidelines.html#sec-git-setup):
...
Sorted from closest experience to Paint, to most generalist / specialised:

1.  [`Drawing`](https://en.ubunlog.com/drawing-alternative-microsoft-paint-ubuntu/)
    — lightweight local Paint-like editor:

    i.  Add Drawing to the `apt` repository:
        `sudo add-apt-repository ppa:cartes/drawing`
    ii. Install with `sudo apt install drawing`

2.  LibreOffice Draw — diagrams and simple page layouts.

3.  [**Photopea**](https://www.photopea.com/) — preferred **free**,
    browser-based Photoshop alternative when you need layers, masks,
    PSD/PSB import, and a large toolset without installing desktop
    software. (Pixlr / “PhotoEditor”-style tools are fine for quick
    crops, but Photopea is more versatile in free mode and closer to a
    full raster editor.)

4.  GIMP (desktop equivalent to a large part of Photoshop’s scope):
...
5.  Inkscape for **vector** graphics and fully editable icons:
...
6.  Background removal for figures / slides: use an online tool such as
    [remove-bg.io](https://remove-bg.io/upload/) when you need a clean
    cut-out of a subject (for example an organism or device photo) before
    composing the panel in Photopea, GIMP, or Inkscape. Prefer this for
    outreach slides; keep raw scientific image panels under version
    control without destructive edits when possible.
...
::: {#wrn-zotero-version .callout-warning}
**Zotero major versions change the Settings UI and feature set.** These
notes assume **Zotero 7** (current stable line as of this writing —
menus under **Edit → Settings…**). If you are still on Zotero 6, labels
differ (Preferences vs Settings). Always check *Help → About Zotero*
before following screenshots, and re-import Better BibTeX preferences
after an upgrade.
:::
...
2.  Retrieve the latest stable build from
    <https://www.zotero.org/download/> (or fetch the archive with
    `curl`).

3.  Extract and link the executable (@lst-zotero-install), then install
    the [Zotero Connector](https://www.zotero.org/download/connectors) in
    your browser:
...
4.  (Optional) Create a desktop launcher at
    `~/.local/share/applications/zotero.desktop` (@lst-zotero-desktop),
    then `chmod +x ~/.local/share/applications/zotero.desktop`.
...
### Better BibTeX and local storage

5.  Install **Better BibTeX** (clean citation keys and `.bib` export):

    -   Download the latest `.xpi` from
        [Better BibTeX](https://retorque.re/zotero-better-bibtex/).
    -   In Zotero: **Tools → Plugins → Install plugin from file…**, then
        restart.
    -   Harmonise citation-key patterns across machines (e.g.
        `[auth:lower][year][journal:lower:abbr]`).
    -   After upgrades, restore settings via
        **Edit → Settings → Better BibTeX → Import preferences**.

::: {#fig-zotero-config layout-ncol="3"}
![Advanced / general settings (keep the library local)](zotero/zotero_advanced_settings.png)

![Better BibTeX citation-key and export preferences](zotero/zotero_better_bibtex_preferences.png)

![Linked / stored file location — prefer a local folder, not OneDrive](zotero/zotero_folder_storage.png)

Core Zotero configuration: keep the SQLite library and stored PDFs on
**local disk** (for example `~/Zotero/`). Avoid syncing the data directory
through OneDrive/Dropbox — use Zotero Sync for metadata and optional file
storage, or linked files under a path that is *not* a cloud-synced tree
used for active Git projects (@wrn-onedrive-exclusive in the OneDrive
appendix).
:::

### Citations in Cursor / Quarto (preferred authoring path)

Favour inserting citations from Zotero **inside Cursor’s Quarto visual
editor**, which can populate the document bibliography automatically via
the Zotero connection
([Quarto visual editor — Zotero citations](https://quarto.org/docs/tools/vscode/visual-editor.html#zotero-citations)):

1.  Enable Zotero in Quarto / Cursor visual mode (Zotero desktop must be
    running; Better BibTeX improves key stability).
2.  Insert citations from the visual editor citation UI — Quarto writes
    `[@citekey]` and can maintain the linked `.bib`.
3.  Keep literature `.bib` files under Git; keep software citations in a
    separate `packages.bib` when your project uses that split.

### Two-way sync with zotcurate (`zotc`) and project rules

[zotcurate](https://github.com/jeetsukumaran/zotcurate) bridges citation
keys in `.qmd` / `.bib` / `.tex` and Zotero **collections**. Project
`.cursorrules` (Bibliography section) encode the hygiene rules: leaf-only
collection membership, allowed BibTeX fields, and
`check_bibliography.py --clean` after every export.

```{mermaid}
%%| label: fig-zotero-flow
%%| fig-cap: "Bidirectional literature workflow (summary)"
%%{init: {'theme': 'neutral'}}%%
flowchart LR
  Z[Zotero + Better BibTeX<br/>local ~/Zotero] -->|export .bib| G[Git literature .bib]
  G -->|check_bibliography --clean| G
  Q[Quarto / Cursor<br/>visual citations] -->|[@keys]| G
  G -->|zotc keys extract| C[zotcurate]
  C -->|collection add leaf-only| Z
  P[packages.bib] -.->|software only| Q
```

![How `.cursorrules` + zotcurate keep Zotero collections and document citekeys aligned across formats](zotero/zotero-zotcurate-sync.png){#fig-zotero-zotcurate-sync}

**Both directions (as used in lab projects):**

1.  **Zotero → Git:** export → `python3 scripts/check_bibliography.py --clean`.
2.  **Git / docs → Zotero:** `zotc keys extract` then
    `collection add|create` (dry-run first); assign each item to
    **exactly one leaf collection**.
3.  Software tools → `packages.bib`, not the Zotero literature library.

### Shortcuts and power options

![Zotero shortcut preferences (restart required after changes)](zotero/zotero-shortcuts.png){#fig-zotero-shortcuts}

Useful defaults (Linux; change under **Edit → Settings → Shortcuts**):

| Action | Typical shortcut |
|--------------------------------------|------------------|
| New item | {{< kbd Ctrl-Shift-N >}} |
| New note | {{< kbd Ctrl-Shift-O >}} |
| Focus libraries pane | {{< kbd Ctrl-Shift-L >}} |
| Quick search | {{< kbd Ctrl-Shift-K >}} |
| Copy citations | {{< kbd Ctrl-Shift-A >}} |
| Copy items | {{< kbd Ctrl-Shift-C >}} |

Additional habits:

-   In **Advanced search** results, hold {{< kbd Ctrl >}} while clicking
    an item to reveal / jump to its location in the library tree.
-   Hold {{< kbd Shift >}} to multi-select a contiguous block of items,
    then batch-update Better BibTeX citekeys (right-click → Better BibTeX
...
![Map of the main sections hard-coded in `Config/.cursor/.cursorrules` (structure, R, bibliography, writing, LaTeX, slides)](../Config/.cursor/cursorrules-map.png){#fig-cursorrules-map}

The figure summarises what those rules enforce for agents and humans:
project layout, R package hygiene, Zotero/bibliography sync, Nature
Methods–style writing, LaTeX semantics, and Quarto reveal.js slidecraft.
:::
